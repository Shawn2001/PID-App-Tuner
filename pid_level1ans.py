...
def PID_control(yspeed, current_pos, target_pos):
    ...
    # Pvalue Data
    Pvalue = error * Kp
    Ivalue += error * timestep
    if Ivalue >= 20:
        Ivalue = 20
    elif Ivalue < -20:
        Ivalue = -20
    Dvalue = Kd * (error - error_prev)
    # print(Dvalue)
    yspeed = yspeed  + Pvalue + Ki * Ivalue + Dvalue
...
def move_ball():
    ...
    # Update the current speed due to gravity
